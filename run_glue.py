...
from modeling_roberta import RobertaSequenceClassification
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The name of the domain task to train on.",
    )
    parser.add_argument(
        "--prompt_length",
        type=int,
    ...
    data_dir = 'dataset/domain_datasets/' if args.file_name in ['CR', 'MR', 'mpqa', 'IMDB'] else 'dataset/'

    args.train_file = data_dir + args.file_name + '/train.csv' if args.file_name else None
    args.validation_file = data_dir + args.file_name + '/dev.csv' if args.file_name else None
    args.test_file = data_dir + args.file_name + '/test.csv' if args.file_name else None
...
def main():
    ...
    fine_tune = 'fine_tune' if args.fine_tune else 'no_f'
    prompting = 'prompting' if args.prompting else 'no_p'
    task_name = args.task_name if args.task_name else args.train_file
    args.experiment_id = task_name + str(args.prompt_length) + fine_tune + prompting + \
    ...
    model = RobertaSequenceClassification.from_pretrained(
        ...
        config=config,
        fine_tune=args.fine_tune, prompting=args.prompting, prompt_length=args.prompt_length
    )
    ...
    def preprocess_function(examples):
        ...
        if args.file_name == 'HP':
            # 不知道为什么dev dataset 里竟然会转出一个None type how？why？impossible？
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.task_name == None:
            for step, batch in enumerate(test_dataloader):
                # when testing, do not pass in labels
                outputs = model(**{"attention_mask": batch["attention_mask"].to(args.device), "input_ids": batch["input_ids"].to(args.device)})
                predictions = outputs[0].argmax(dim=-1) if not is_regression else outputs[0].squeeze()
                metric.add_batch(
                    predictions=accelerator.gather(predictions),
                    references=accelerator.gather(batch["labels"]),
                )
